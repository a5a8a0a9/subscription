...
describe('billing date utilities', () => {
	...
	it('supports custom day intervals', () => {
		const events = billingEventsBetween(
			item({
				...
				billingCycle: { kind: 'custom', interval: 10, unit: 'day' },
			}),
	...
	it('normalizes common cycles to a monthly amount', () => {
		expect(
			monthlyEquivalent(
				item({ amount: 3600, billingCycle: { kind: 'yearly' } }),
			),
		).toBe(300);
		expect(
			monthlyEquivalent(
				item({ amount: 900, billingCycle: { kind: 'quarterly' } }),
			),
		).toBe(300);
		expect(
			monthlyEquivalent(
				item({ amount: 300, billingCycle: { kind: 'monthly' } }),
			),
		).toBe(300);
	});
});
