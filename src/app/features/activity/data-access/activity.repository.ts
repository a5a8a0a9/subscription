		...
		try {
			...
			if (legacyRaw) {
				...
				if (legacy.version !== 1 || !Array.isArray(legacy.items))
					throw new Error('Unsupported legacy data');
				const migrated = legacy.items
					.map((item) => ({
						...item,
						startDate: item.startDate ?? item.nextBillingDate,
					}))
					.filter(isActivity);
				this.save(migrated);
				return migrated;
		...
		return [
			{
				...
				nextBillingDate: formatLocalDate(addDays(today, 3)),
			...
			},
			{
				...
				nextBillingDate: formatLocalDate(addDays(today, 12)),
				billingCycle: { kind: 'monthly' },
				reminderDays: 3,
			...
			},
			{
				...
				nextBillingDate: formatLocalDate(addDays(today, 24)),
				billingCycle: { kind: 'yearly' },
				reminderDays: 7,
				website: '',
				notes: '年度備份方案',
				createdAt: now,
