...
describe('ActivityRepository', () => {
	...
	it('migrates v1 data by using the next billing date as the start date', () => {
		localStorage.setItem('activity-manager.activities.v1', JSON.stringify({
			version: 1,
			items: [{
				id: 'legacy', name: 'Legacy', amount: 100, category: '其他', status: 'active',
				nextBillingDate: '2026-09-01', billingCycle: { kind: 'monthly' }, reminderDays: 3,
		...
		expect(JSON.parse(localStorage.getItem('activity-manager.activities.v2') ?? '{}').version).toBe(2);
	});
});
